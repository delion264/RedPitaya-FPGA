...
(
  ...
  `else
  // ---------------------------------------------------------------------------
  // DDC multi-tuner SDR on sys[6] @ 0x4060_0000 (replaces the stub).
  //   sys_bus -> AXI4-Lite bridge -> ddc_top (4 dual-mode slots, VHDL).
  // ADC channel-A tap, free-running (adc_valid=1). IQ egress (packetizer + DMA)
  // is integration increment 2 -- the IQ stream is sunk here (m_tready=1).
  // ---------------------------------------------------------------------------
  logic [11:0] ddc_awaddr, ddc_araddr;
  logic [ 2:0] ddc_awprot, ddc_arprot;
  logic        ddc_awvalid, ddc_awready, ddc_wvalid, ddc_wready, ddc_bvalid, ddc_bready;
  logic        ddc_arvalid, ddc_arready, ddc_rvalid, ddc_rready;
  logic [31:0] ddc_wdata, ddc_rdata;
  logic [ 3:0] ddc_wstrb;
  logic [ 1:0] ddc_bresp, ddc_rresp;

  sysbus_to_axilite u_ddc_bridge (
    .bus      (sys[6]),
    .m_awaddr (ddc_awaddr), .m_awprot(ddc_awprot), .m_awvalid(ddc_awvalid), .m_awready(ddc_awready),
    .m_wdata  (ddc_wdata ), .m_wstrb (ddc_wstrb ), .m_wvalid (ddc_wvalid ), .m_wready (ddc_wready ),
    .m_bresp  (ddc_bresp ), .m_bvalid(ddc_bvalid), .m_bready (ddc_bready ),
    .m_araddr (ddc_araddr), .m_arprot(ddc_arprot), .m_arvalid(ddc_arvalid), .m_arready(ddc_arready),
    .m_rdata  (ddc_rdata ), .m_rresp (ddc_rresp ), .m_rvalid (ddc_rvalid ), .m_rready (ddc_rready )
  );

  ddc_top i_ddc (            // NUM_SLOTS defaults to 4 in the VHDL
    .clk          (adc_clk ), .resetn      (adc_rstn),
    .s_axi_awaddr (ddc_awaddr), .s_axi_awprot(ddc_awprot), .s_axi_awvalid(ddc_awvalid), .s_axi_awready(ddc_awready),
    .s_axi_wdata  (ddc_wdata ), .s_axi_wstrb (ddc_wstrb ), .s_axi_wvalid (ddc_wvalid ), .s_axi_wready (ddc_wready ),
    .s_axi_bresp  (ddc_bresp ), .s_axi_bvalid(ddc_bvalid), .s_axi_bready (ddc_bready ),
    .s_axi_araddr (ddc_araddr), .s_axi_arprot(ddc_arprot), .s_axi_arvalid(ddc_arvalid), .s_axi_arready(ddc_arready),
